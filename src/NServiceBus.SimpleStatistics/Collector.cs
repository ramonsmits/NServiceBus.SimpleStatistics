...
public class Collector : Implementation
{
    ...
    public Collector()
    {
        ...
        _outputLog = string.Equals(ConfigurationManager.AppSettings[SimpleStatisticsFeature.SettingKey + "/OutputLog"], bool.TrueString, StringComparison.InvariantCultureIgnoreCase);

        int interval;
        if (int.TryParse(ConfigurationManager.AppSettings[SimpleStatisticsFeature.SettingKey + "/IntervalMilliSeconds"], out interval)) _updateInMilliSeconds = interval;
